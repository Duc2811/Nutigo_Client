...
import {
  ...
  deleteMessage,
  getChatUsers,
} from "../../Service/Client/ApiChatbox";
import { sendMessageToAI } from "../../Service/Client/APIChatbot";
...
const Chatbox = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (selectedUser) {
      if (selectedUser.isAI) {
        // Reset messages for AI chat since it doesn't have a persistent history
        setMessages([]);
      } else {
        loadChatHistory();
      }
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      if (selectedUser.isAI) {
        // Use sendMessageToAI for AI chat
        const data = await sendMessageToAI(newMessage);
        const aiReply = {
          id: `ai-${Date.now()}`,
        ...
        // Send message to regular user using sendMessage
  ...
  const getMessageActions = (message) => {
    if (!message.isCurrentUser || selectedUser?.isAI) return null;
  ...
  return (
    ...
    >
              ...
              <List
                ...
                renderItem={(user) => (
                  ...
                  >
                    <List.Item.Meta
                      avatar={
                        <Avatar
                          src={user.avatar}
                          style={{
                            backgroundColor: user.isAI ? "#722ed1" : "#1890ff",
                          }}
                          icon={<User size={16} />}
                        />
                      }
                      title={
                        ...
                        >
                          ...
                          {user.isAI && (
                            <span
                              style={{ color: "#722ed1", marginLeft: "8px" }}
                            >
                              AI Assistant
                            </span>
                          )}
                    ...
                    />
                    <Badge status={user.isAI ? "processing" : "success"} />
                  </List.Item>
                )}
              />
            )}
              ...
              >
                ...
                >
                  <Avatar
                    ...
                    style={{
                      backgroundColor: selectedUser.isAI
                        ? "#722ed1"
                        : "#1890ff",
                    }}
                    icon={<User size={16} />}
                  ...
                  <div>
                    ...
                    >
                      {selectedUser.isAI ? "AI Assistant" : "Online"}
                    </p>
                  </div>
                </div>
              </div>
      ...
      <Popconfirm
        ...
        onConfirm={() =>
          handleDeleteMessage(messageToDelete?.id || messageToDelete?._id)
        }
